...
static void shellSort(usize iThread, visualizer_array arrayHandle, visualizer_int* array, usize n, const usize* gaps, usize nGaps) {
	...
	for (--pass; pass >= 0; --pass) {
		...
		visualizer_marker pointer = Visualizer_CreateMarker(iThread, arrayHandle, gap, Visualizer_MarkerAttribute_Pointer);
		for (usize i = gap; i < n; ++i) {
			visualizer_int temp = array[i];
			Visualizer_UpdateRead(iThread, arrayHandle, i, 1.0f);
			Visualizer_MoveMarker(iThread, &pointer, i);

			usize j;
			for (j = i; j >= gap; j -= gap) {
				Visualizer_UpdateRead(iThread, arrayHandle, j - gap, 1.0f);
				if (array[j - gap] <= temp)
					break;
				Visualizer_UpdateWrite(iThread, arrayHandle, j, array[j - gap], 1.0f);
				array[j] = array[j - gap];
			}
...
void ShellSortTokuda(usize iThread, visualizer_array arrayHandle, visualizer_int* array, usize n) {
	Visualizer_SetAlgorithmSleepMultiplier(
		Visualizer_ScaleSleepMultiplier(n, 0.25f, Visualizer_SleepScale_NLogN)
	);
	shellSort(iThread, arrayHandle, array, n, gapsTokuda, static_arrlen(gapsTokuda));
	return;
}
...
static void gappedInsertion(usize iThread, void* parameter) {
	insertion_parameter* insertionParameter = parameter;
	visualizer_array arrayHandle = insertionParameter->arrayHandle;
	visualizer_int* array = insertionParameter->array;
	usize start = insertionParameter->start;
	usize end = insertionParameter->end;
	usize gap = insertionParameter->gap;
	atomic_store_fence_light(&insertionParameter->parameterRead, true);
	insertionParameter = NULL;

	visualizer_marker pointer = Visualizer_CreateMarker(iThread, arrayHandle, start + gap, Visualizer_MarkerAttribute_Pointer);
	for (usize i = start + gap; i < end; i += gap) {
		visualizer_int temp = array[i];
		Visualizer_UpdateRead(iThread, arrayHandle, i, 1.0f);
		Visualizer_MoveMarker(iThread, &pointer, i);
		...
		for (j = i; j >= start + gap; j -= gap) {
			...
			if (temp2 <= temp)
				break;
			Visualizer_UpdateWrite(iThread, arrayHandle, j, temp2, 1.0f);
			array[j] = temp2;
		}
		Visualizer_UpdateWrite(iThread, arrayHandle, j, temp, 1.0f);
		array[j] = temp;
...
void ShellSortParallel(usize iThread, visualizer_array arrayHandle, visualizer_int* array, usize n) {
	Visualizer_SetAlgorithmSleepMultiplier(
		Visualizer_ScaleSleepMultiplier(n, 0.25f, Visualizer_SleepScale_N) // TODO FIXME
	);

	usize nGaps = static_arrlen(gapsTokuda);
	const usize* gaps = gapsTokuda;
